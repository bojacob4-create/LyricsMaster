...
def _render(artist, title, excerpt_lines, artwork_img, deep_link):
    ...
    # Round-86: warm-tinted soft drop shadow behind the QR tile, offset
    # down-right so the tile lifts off the background.
    qr_shadow = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    ...
    # modules keep full scan contrast over it.
    qr_tile_rgb = _mix(glow_rgb, (48, 30, 20), 0.55)
    ...
    # Round-86: warm-tinted soft drop shadow so the cover lifts off the
    # background instead of sitting flat. Warm near-black keeps it in the
    # palette family — no cold pure-black shadow.
    art_shadow = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    ImageDraw.Draw(art_shadow).rounded_rectangle(
        [ax + 10, ay0 + 16, ax + 310, ay0 + 316],
        radius=40, fill=(12, 6, 4, 140))
    img.alpha_composite(art_shadow.filter(ImageFilter.GaussianBlur(20)))
